...
utils.resourceStatusImage = function (status) {
    if (status === 'notDownloaded')
        return utils.notDownloadedResource();
...
utils.resourceStatusFile = function (status) {
    ...
    if (status === 'downloading')
        return utils.loaderImage();
    if (status === 'encrypted')
        return utils.loaderImage();
    if (status === 'error')
        return utils.errorImage();
...
utils.imageReplacement = function (ResourceModel, src, resources, resourceBaseUrl, itemIdToUrl = null) {
    if (!ResourceModel || !resources)
        return null;
    if (!ResourceModel.isResourceUrl(src))
        return null;
