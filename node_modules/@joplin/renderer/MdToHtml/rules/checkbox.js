...
function createPrefixTokens(Token, id, checked, label, postMessageSyntax, sourceToken, disabled) {
    let token = null;
    const tokens = [];
...
function checkboxPlugin(markdownIt, options) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (currentListItem && currentListItem.markup === '-' && !processedFirstInline && token.type === 'inline') {
                ...
                if (renderingType === 1) {
                    ...
                    if (listClass.indexOf('joplin-') < 0)
                        currentList.attrSet('class', (`${listClass} joplin-checklist`).trim());
                    if (checked) {
