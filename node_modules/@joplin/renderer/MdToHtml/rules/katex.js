...
function stringifyKatexOptions(options) {
    if (!options)
        return '';
    ...
    if (options.macros) {
        ...
        for (const k of Object.keys(options.macros)) {
            const macroText = options.macros[k].tokens.map((t) => t.text);
...
const cache_ = {};
function renderToStringWithCache(latex, katexOptions) {
    ...
    if (cacheKey in cache_) {
        ...
        if (beforeMacros === afterMacros)
            cache_[cacheKey] = output;
        return output;
    }
}
function renderKatexError(latex, error) {
...
exports.default = {
    plugin: function (markdownIt, options) {
        ...
        if (!options.context.userData.__katex)
            options.context.userData.__katex = { macros: {} };
        const katexOptions = {};
