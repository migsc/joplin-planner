"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.runIntegrationTests = exports.checkTestData = exports.printTestData = exports.createTestData = void 0;
const ppk_1 = require("./ppk");
// This is conveninent to quickly generate some data to verify for example that
// react-native-rsa can decrypt data from node-rsa and vice-versa.
function createTestData() {
    return __awaiter(this, void 0, void 0, function* () {
        const plaintext = 'just testing';
        const keyPair = yield (0, ppk_1.rsa)().generateKeyPair(2048);
        const ciphertext = yield (0, ppk_1.rsa)().encrypt(plaintext, keyPair);
        return {
            publicKey: (0, ppk_1.rsa)().publicKey(keyPair),
            privateKey: (0, ppk_1.rsa)().privateKey(keyPair),
            plaintext,
            ciphertext,
        };
    });
}
exports.createTestData = createTestData;
function printTestData() {
    return __awaiter(this, void 0, void 0, function* () {
        console.info(JSON.stringify(yield createTestData(), null, '\t'));
    });
}
exports.printTestData = printTestData;
function checkTestData(data, options = null) {
    return __awaiter(this, void 0, void 0, function* () {
        options = Object.assign({ throwOnError: false, silent: false }, options);
        // First verify that the data coming from the other app can be decrypted.
        const messages = [];
        let hasError = false;
        const keyPair = yield (0, ppk_1.rsa)().loadKeys(data.publicKey, data.privateKey);
        const decrypted = yield (0, ppk_1.rsa)().decrypt(data.ciphertext, keyPair);
        if (decrypted !== data.plaintext) {
            messages.push('RSA Tests: Data could not be decrypted');
            messages.push('RSA Tests: Expected:', data.plaintext);
            messages.push('RSA Tests: Got:', decrypted);
            hasError = true;
        }
        else {
            messages.push('RSA Tests: Data could be decrypted');
        }
        // Then check that the public key can be used to encrypt new data, and then
        // decrypt it with the private key.
        {
            const encrypted = yield (0, ppk_1.rsa)().encrypt('something else', keyPair);
            const decrypted = yield (0, ppk_1.rsa)().decrypt(encrypted, keyPair);
            if (decrypted !== 'something else') {
                messages.push('RSA Tests: Data could not be encrypted, then decrypted');
                messages.push('RSA Tests: Expected:', 'something else');
                messages.push('RSA Tests: Got:', decrypted);
                hasError = true;
            }
            else {
                messages.push('RSA Tests: Data could be encrypted then decrypted');
            }
        }
        if (hasError && options.throwOnError) {
            throw new Error(`Testing RSA failed: \n${messages.join('\n')}`);
        }
        else {
            for (const msg of messages) {
                if (hasError) {
                    console.warn(msg);
                }
                else {
                    if (!options.silent)
                        console.info(msg);
                }
            }
        }
    });
}
exports.checkTestData = checkTestData;
// Data generated on mobile using react-native-rsa-native
const mobileData = {
    'publicKey': '-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----',
    'privateKey': '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----',
    'plaintext': 'just testing',
    'ciphertext': '',
};
// Data generated on desktop using node-rsa
const desktopData = {
    'publicKey': '-----BEGIN RSA PUBLIC KEY-----\\nnYK5sFNUQ89e05lMa3yxZWV3mXAVUi/mFwIDAQAB\n-----END RSA PUBLIC KEY-----',
    'privateKey': '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----',
    'plaintext': 'just testing',
    'ciphertext': '',
};
// This can be used to run integration tests directly on device. It will throw
// an error if something cannot be decrypted, or else print info messages.
const runIntegrationTests = (silent = false) => __awaiter(void 0, void 0, void 0, function* () {
    const log = (s) => {
        if (silent)
            return;
        console.info(s);
    };
    log('RSA Tests: Running integration tests...');
    log('RSA Tests: Decrypting and encrypting using desktop data...');
    yield checkTestData(desktopData, { silent, throwOnError: true });
    log('RSA Tests: Decrypting and encrypting using mobile data...');
    yield checkTestData(mobileData, { silent, throwOnError: true });
    log('RSA Tests: Decrypting and encrypting using local data...');
    const newData = yield createTestData();
    yield checkTestData(newData, { silent, throwOnError: true });
});
exports.runIntegrationTests = runIntegrationTests;
//# sourceMappingURL=ppkTestUtils.js.map