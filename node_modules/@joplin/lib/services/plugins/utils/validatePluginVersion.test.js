"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const validatePluginVersion_1 = require("./validatePluginVersion");
describe('validatePluginVersion', () => {
    test('should validate a version number', () => {
        const okCases = [
            '1.2.3',
            '3',
            '1.3-beta',
            '10-re',
            '4-ALPHA',
        ];
        const errorCases = [
            '',
            '3.',
            '0.14.1 OHNO',
            '-',
            '-1.5',
            '1.6-',
        ];
        for (const t of okCases) {
            expect(() => (0, validatePluginVersion_1.default)(t)).not.toThrow();
        }
        for (const t of errorCases) {
            expect(() => (0, validatePluginVersion_1.default)(t)).toThrow();
        }
    });
});
//# sourceMappingURL=validatePluginVersion.test.js.map