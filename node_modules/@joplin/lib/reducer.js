...
exports.stateUtils = exports.MAX_HISTORY = exports.store = exports.setStore = exports.defaultState = void 0;
...
exports.defaultState = {
    ...
    needApiAuth: false,
    profileConfig: null,
    pluginService: reducer_1.defaultState,
    shareService: reducer_2.defaultState,
};
...
const createShallowArrayEqualSelector = createSelectorCreator(defaultMemoize, (prev, next) => {
...
const selectArrayShallow = createCachedSelector((state) => state.array, (array) => array)({
...
function handleItemDelete(draft, action) {
    ...
    const selectedItemKeys = isSingular ? [draft[selectedItemKey]] : draft[selectedItemKey];
    ...
    draft[listKey] = newItems;
    const newIds = [];
    for (let i = 0; i < newSelectedIndexes.length; i++) {
        newIds.push(newItems[newSelectedIndexes[i]].id);
    }
    draft[selectedItemKey] = isSingular ? newIds[0] : newIds;
...
const reducer = (0, immer_1.default)((draft = exports.defaultState, action) => {
            ...
            case 'NOTE_UPDATE_ONE':
                {
                    ...
                    const isViewingConflictFolder = draft.notesParentType === 'Folder' && draft.selectedFolderId === Folder_1.default.conflictFolderId();
                    const noteIsInFolder = function (note, folderId) {
                        if (note.is_conflict && isViewingConflictFolder)
                            return true;
                        if (!('parent_id' in modNote) || note.parent_id === folderId)
                            return true;
                        return false;
                    };
                    ...
                    newNotes = Note_1.default.sortNotes(newNotes, exports.stateUtils.notesOrder(draft.settings), draft.settings.uncompletedTodosOnTop);
            ...
            case 'TAG_UPDATE_ONE':
                {
                    ...
                    if (selectedNoteHasTag)
                        updateOneItem(draft, action, 'selectedNoteTags');
                }
                break;
