...
function wrapError(prefix, error) {
    ...
    const newError = new Error([prefix, error.message || ''].join(': '));
    if ('name' in error)
        newError.name = error.name;
    if ('fileName' in error)
        newError.fileName = error.fileName;
    if ('lineNumber' in error)
        newError.lineNumber = error.lineNumber;
    if ('columnNumber' in error)
        newError.columnNumber = error.columnNumber;
    ...
    if ('stack' in error)
        newError.stack = [prefix, error.stack].join(': ');
    return newError;
}
