...
function decodeBase64File(sourceFilePath, destFilePath) {
...
function fuzzyMatch(note) {
    return __awaiter(this, void 0, void 0, function* () {
        if (note.created_time < time_1.default.unixMs() - 1000 * 60 * 60 * 24 * 360) {
            const notes = yield Note_1.default.modelSelectAll('SELECT * FROM notes WHERE is_conflict = 0 AND created_time = ? AND title = ?', [note.created_time, note.title]);
            return notes.length !== 1 ? null : notes[0];
        }
        const notes = yield Note_1.default.modelSelectAll('SELECT * FROM notes WHERE is_conflict = 0 AND created_time = ?', [note.created_time]);
        if (notes.length === 0)
            return null;
        if (notes.length === 1)
            return notes[0];
...
function processNoteResource(resource) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!resource.hasData) {
            ...
            yield fs.writeFile(resource.dataFilePath, '');
        }
        else {
            if (resource.dataEncoding === 'base64') {
                const decodedFilePath = `${resource.dataFilePath}.decoded`;
                yield decodeBase64File(resource.dataFilePath, decodedFilePath);
            ...
            if (!resource.id) {
                ...
                resource.id = yield md5FileAsync(resource.dataFilePath);
            }
...
function saveNoteResources(note) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < note.resources.length; i++) {
            ...
            // same resource.
            const existingResource = yield Resource_1.default.load(toSave.id);
            if (existingResource)
                continue;
            yield fs.move(resource.dataFilePath, Resource_1.default.fullPath(toSave), { overwrite: true });
            yield Resource_1.default.save(toSave, { isNew: true });
            resourcesCreated++;
        }
        return resourcesCreated;
    });
}
function saveNoteTags(note) {
    return __awaiter(this, void 0, void 0, function* () {
        let notesTagged = 0;
        for (let i = 0; i < note.tags.length; i++) {
            const tagTitle = note.tags[i];
            let tag = yield Tag_1.default.loadByTitle(tagTitle);
            if (!tag)
                tag = yield Tag_1.default.save({ title: tagTitle });
...
function saveNoteToStorage(note, importOptions) {
    return __awaiter(this, void 0, void 0, function* () {
        importOptions = Object.assign({}, {
            fuzzyMatching: false,
        }, importOptions);
        note = Note_1.default.filter(note);
        const existingNote = importOptions.fuzzyMatching ? yield fuzzyMatch(note) : null;
        const result = {
            noteCreated: false,
            noteUpdated: false,
            noteSkipped: false,
        ...
        const resourcesCreated = yield saveNoteResources(note);
        result.resourcesCreated += resourcesCreated;
        const notesTagged = yield saveNoteTags(note);
        result.notesTagged += notesTagged;
        if (existingNote) {
            const diff = BaseModel_1.default.diffObjects(existingNote, note);
            delete diff.tags;
            ...
            yield Note_1.default.save(diff, { autoTimestamp: false });
...
const preProcessFile = (filePath) => __awaiter(void 0, void 0, void 0, function* () {
...
function importEnex(parentFolderId, filePath, importOptions = null) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fileToProcess = yield preProcessFile(filePath);
                    ...
                    while (notes.length) {
                        ...
                        try {
                            for (let i = 0; i < note.resources.length; i++) {
                                ...
                                try {
                                    resource = yield processNoteResource(resource);
                                }
                                catch (error) {
                            ...
                            const body = importOptions.outputFormat === 'html' ?
                                yield enexXmlToHtml(note.bodyXml, note.resources) :
                                yield (0, import_enex_md_gen_1.enexXmlToMd)(note.bodyXml, note.resources);
                            delete note.bodyXml;
                            ...
                            const result = yield saveNoteToStorage(note, importOptions);
...
//# sourceMappingURL=import-enex.js.map