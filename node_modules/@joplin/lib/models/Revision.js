...
class Revision extends BaseItem_1.default {
    ...
    static countRevisions(itemType, itemId) {
        return __awaiter(this, void 0, void 0, function* () {
            const r = yield this.db().selectOne('SELECT count(*) as total FROM revisions WHERE item_type = ? AND item_id = ?', [itemType, itemId]);
            return r ? r.total : 0;
        });
    }
    ...
    static itemsWithRevisions(itemType, itemIds) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!itemIds.length)
                return [];
            const rows = yield this.db().selectAll(`SELECT distinct item_id FROM revisions WHERE item_type = ? AND item_id IN ("${itemIds.join('","')}")`, [itemType]);
            return rows.map((r) => r.item_id);
        });
    }
    static itemsWithNoRevisions(itemType, itemIds) {
        return __awaiter(this, void 0, void 0, function* () {
            const withRevs = yield this.itemsWithRevisions(itemType, itemIds);
            const output = [];
    ...
    static mergeDiffs(revision, revs = null) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!revs) {
                revs = yield this.modelSelectAll('SELECT * FROM revisions WHERE item_type = ? AND item_id = ? AND item_updated_time <= ? ORDER BY item_updated_time ASC', [revision.item_type, revision.item_id, revision.item_updated_time]);
            }
            else {
                revs = revs.slice();
            }
    ...
    static deleteOldRevisions(ttl) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const revisions = yield this.modelSelectAll('SELECT * FROM revisions WHERE item_updated_time < ? ORDER BY item_updated_time DESC', [cutOffDate]);
            const doneItems = {};
            for (const rev of revisions) {
                const doneKey = `${rev.item_type}_${rev.item_id}`;
                if (doneItems[doneKey])
                    continue;
                const keptRev = yield this.modelSelectOne('SELECT * FROM revisions WHERE item_updated_time >= ? AND item_type = ? AND item_id = ? ORDER BY item_updated_time ASC LIMIT 1', [cutOffDate, rev.item_type, rev.item_id]);
                try {
                    ...
                    if (!keptRev) {
                        const hasEncrypted = yield this.modelSelectOne(`SELECT * FROM revisions WHERE encryption_applied = 1 AND ${deleteQueryCondition}`, deleteQueryParams);
                        if (hasEncrypted)
                            throw new JoplinError_1.default('One of the revision to be deleted is encrypted', 'revision_encrypted');
                        yield this.db().transactionExecBatch([deleteQuery]);
                    }
                    else {
                        ...
                        const merged = yield this.mergeDiffs(keptRev);
    ...
    static revisionExists(itemType, itemId, updatedTime) {
        return __awaiter(this, void 0, void 0, function* () {
            const existingRev = yield Revision.latestRevision(itemType, itemId);
